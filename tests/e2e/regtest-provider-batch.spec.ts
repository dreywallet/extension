...
async function createProviderWallet(input: {
  ...
  const settings = await input.extensionContext.newPage();
  try {
    await settings.goto(
      `chrome-extension://${input.extensionId}/fullpage.html#/settings/wallets-accounts`,
    );
    await expect(settings.getByLabel('Active account')).toBeEnabled();
    const enabled = await settings.evaluate(async () => {
      ...
      if (typeof session?.vaultId !== 'string' || typeof session.sessionId !== 'string') return false;
...
test('@extended signs two real independent provider PSBTs in one review and never broadcasts', async ({
  ...
  const paymentAddress = await createProviderWallet({
    onboarding, popup, extensionContext, extensionId, name: 'Provider batch regtest E2E',
  });
  ...
  await approval.approve({ password: TEST_PASSWORD, confirmation: 'SIGN PSBT' });
...
test('@extended rejects duplicate and conflicting real provider inputs before approval', async ({
  onboarding,
  popup,
  dapp,
  extensionContext,
  ...
  await assertRegtestReady();
  const paymentAddress = await createProviderWallet({
    onboarding, popup, extensionContext, extensionId, name: 'Provider rejection regtest E2E',
  });
  ...
  const conflicting = await createProviderPsbtFixture({
    funding: funding!,
  ...
  for (const fixtures of [[first!, first!], [first!, conflicting]]) {
    const pageCount = extensionContext.pages().length;
    await dapp.configureTransactionBatch(batchRequest(paymentAddress, fixtures));
    await dapp.invoke('Sign transaction batch');
    await expect(dapp.output()).toContainText('Method is not supported by this account', {
      timeout: 60_000,
    });
    const error = await dapp.outputJson<ProviderErrorResult>();
    expect(error.error).toMatchObject({ code: -32001 });
...
test('@extended returns no batch result when a real input is spent during approval', async ({
  ...
  await assertRegtestReady();
  const paymentAddress = await createProviderWallet({
    onboarding, popup, extensionContext, extensionId, name: 'Provider stale regtest E2E',
  });
  ...
  await conflictApproval.approve({ password: TEST_PASSWORD, confirmation: 'SIGN PSBT' });
  await expect(dapp.output()).toContainText('psbtBase64', { timeout: 60_000 });
