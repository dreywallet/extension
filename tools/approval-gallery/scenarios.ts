...
export const APPROVAL_GALLERY_SCENARIOS: readonly ApprovalGalleryScenario[] = [
  ...
  },
  {
    ...
    label: 'Linked sale or recovery',
    description:
      'A three-PSBT listing graph with sale and recovery alternatives that share funding.',
    snapshot: request(
      '20000000-0000-4000-8000-000000000008', 'signMultipleTransactions', 'https://ordnet.example',
      {
        ...
        transactions: [
          ...
          },
          {
            ...
            approvalExplanation: approvalExplanation({
              ...
              sighashes: [{ inputIndex: 0, raw: 129, name: 'ALL|ANYONECANPAY',
                inputSet: 'changeable', outputs: 'all', correspondingOutputIndex: null,
                fee: 'changeable' }],
              outputs: [{ valueSats: '9500', address: 'Recovered inscription', ownership: 'wallet',
                role: 'ordinal_change', commitment: 'fixed' }],
            }),
            security: { psbtHash: 'f0'.repeat(32), psbtBytes: 6 }, signingInputs: [{ index: 0, sighash: 129 }],
          },
        ],
      },
    ),
